...
def get_token_data(contract_address, chain_id="1"):
    ...
    try:
        ...
        data = response.json()
        print(f"GoPlus response: {data}")  # Debug log
        ...
        if data.get("code") == "0" and "result" in data:
            ...
            print(f"GoPlus returned error code: {data.get('code')}")
            return None
...
def analyze_token(contract_address, chain_id="1"):
    ...
    analysis_data = {
        ...
        "chain_id": chain_id,
        "token_name": get_val("token_name", "Unknown Token"),
        "token_symbol": get_val("token_symbol", "UNKNOWN"),
        "decimals": get_val("decimals", "18"),
        
        # Red flags
        "is_honeypot": get_val("is_honeypot") == "1",
        "owner_change_balance": get_val("owner_change_balance") == "1",
        "cannot_sell_all": get_val("cannot_sell_all") == "1",
        "slippage_modifiable": get_val("slippage_modifiable") == "1",
        "is_blacklisted": get_val("is_blacklisted") == "1",
        "is_whitelisted": get_val("is_whitelisted") == "1",
        "is_proxy": get_val("is_proxy") == "1",
        "is_mintable": get_val("is_mintable") == "1",
        "owner_address": get_val("owner_address", "Not found"),
        "creator_address": get_val("creator_address", "Not found"),
        
        # Supply/Liquidity
        "total_supply": get_val("total_supply", "Unknown"),
        "transfer_pausable": get_val("transfer_pausable") == "1",
        "buy_tax": get_val("buy_tax", "Unknown"),
        "sell_tax": get_val("sell_tax", "Unknown"),
        "liquidity_holder_count": get_val("liquidity_holder_count", "0"),
        "is_open_source": get_val("is_open_source") == "1",
        "trading_cooldown": get_val("trading_cooldown") == "1",
